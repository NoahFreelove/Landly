...
from models import User, Unit, Payment, KlarnaDebt, Market, ResourceMetric, Notification
from schemas import UserResponse, UnitResponse, PaymentResponse, KlarnaDebtResponse, MarketResponse, NotificationResponse, EvictionStatusResponse
...
def get_dashboard(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    resources = db.query(ResourceMetric).filter(ResourceMetric.unit_id == user.unit_id).all() if user.unit_id else []
    ...
    eviction_status = {
        ...
        "deadline": None,
        "reason": "Outstanding debt exceeds threshold" if total_overdue > 1000 else None,
    ...
    gentrification_index = min(100, (avg_rent / 6200) * 100)  # 6200 is max rent (Suite-1A Platinum)

    return {
        "user": UserResponse.model_validate(user),
        "unit": UnitResponse.model_validate(unit) if unit else None,
        "resources": [{"unit_id": r.unit_id, "metric_type": r.metric_type, "current_value": r.current_value, "max_value": r.max_value, "status": r.status, "trend": r.trend} for r in resources],
        ...
        "credit_score": 300 + int(user.social_credit_score * 0.55),  # Map 0-1000 to 300-850
        "interest_rate": 5.5 + max(0, (700 - user.social_credit_score) * 0.02)  # Higher rate for lower scores
    }
